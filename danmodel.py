...
def feature_extractor(input_op, reuse):
...
def label_predictor(resh1, reuse, training=False):
    ...
    fc8 = fc_op(fc6, name="fc8", n_out=3,reuse=reuse, reluBool = False)
    return fc8
	
def domain_predictor(resh1, reuse):
	#l=1
	#feat = flip_gradient(resh1, l)
	feat=resh1
	d_fc1 = fc_op(feat, name="d_fc1", n_out=100,reuse=reuse)
	d_fc2 = fc_op(feat, name="d_fc2", n_out=100,reuse=reuse)
	d_fc3 = fc_op(d_fc1, name="d_fc3", n_out=2,reuse=reuse, reluBool = False)
	return d_fc3
...
def loss_domain(d_logits, d_gt):
	return tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=d_logits,labels=d_gt))
...
def noise_image_precropped(image):
	""" image -> image
		given an image, apply a noise """
	offset_height =  tf.random_uniform([], minval=21, maxval=27, dtype=tf.int32)
	offset_width =  tf.random_uniform([], minval=15, maxval=22, dtype=tf.int32)
	im_size = tf.random_uniform([], minval=35, maxval=42, dtype=tf.int32)
	image_output = tf.image.crop_to_bounding_box(image, offset_height, offset_width, im_size, im_size)
	image_output = tf.image.resize_images(image_output, IMAGE_SIZE, IMAGE_SIZE)
	image_output = tf.slice(image_output, [0,0,0], [IMAGE_SIZE, IMAGE_SIZE, 1])
	return image_output
...
def noise_batch(images):
	...
	return(tf.map_fn(noise_image_precropped, images))
